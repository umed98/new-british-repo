...
<head>
    ...
    <title>AllenHouse Jhansi| Video Gallery</title>
...
<body>
                ...
                <ol class="inline-flex items-center space-x-1 md:space-x-2 rtl:space-x-reverse">
                    ...
                    <li>
                        <div class="flex items-center">
                            ...
                            <a href="video-gallery.php" class="ms-1 text-sm font-medium text-blue-main">Video
                                Gallery</a>
                        </div>
                    </li>
                </ol>
    ...
    <?php include "includes/foot.php" ?>
     <script>
        let videoData = []; // global variable to store video gallery data

        async function loadGallery() {
            try {
                const response = await fetch('https://allenhouseadmin.fastranking.tech/api/video-gallery/SCLID000004');
                const data = await response.json();
                videoData = data.data; // save for search
                renderGallery(videoData); // initial render
            } catch (error) {
        ...
        function renderGallery(data) {
            ...
            data.forEach(item => {
                const media = item.videogallerymedia[0];
                if (!media || !media.url) return;

                const rawUrl = media.url;
                const youtubeMatch = rawUrl.match(/(?:youtube\.com\/watch\?v=|youtu\.be\/)([^\s&]+)/);
                const videoUrl = youtubeMatch ? `https://www.youtube.com/embed/${youtubeMatch[1]}` : rawUrl;
                        ...
                        <div class="flex gap-4">
                            ...
                            <div class="w-[70%]">
                                ...
                                <div class="flex gap-2 text-[9px] text-[#3B3B3B] m-2">
                                    ...
                                    <div>Total Video(s): <strong>${item.videogallerymedia.length}</strong></div>
                                </div>
                            </div>
                        </div>
        ...
        // Search input filtering across title, type, and date
