...
const [data, setData] = useState(initialData); // Copy of your paginatedData
  ...
  // Helper Functions
  const parsewishlist = (wishlist) => parseFloat(wishlist.replace(/[^0-9.]/g, "")) || 0;
  ...
  const sortedData = useMemo(() => {
    ...
    if (cartFilter !== "All Orders") {
      sortableData = sortableData.filter((item) => item.cart === cartFilter);
    }
    ...
    if (sortConfig.key) {
      sortableData.sort((a, b) => {
       ...
       if (sortConfig.key === "customerId" || sortConfig.key === "orders") {
    ...
    return sortableData;
  }, [data, searchTerm, sortConfig, cartFilter]);
...
const confirmDelete = () => {
  setData(prevData => prevData.filter(item => item.customerId !== customerToDelete));
  setModalOpen(false);
              ...
              <table className="w-full table-auto min-w-max">
                ...
                <tbody>
                  {paginatedData.map((row) => (
                    ...
                    >
                      <td className="p-3 px-6 w-[50px] whitespace-nowrap">
                        {row.customerId}
                      </td>
